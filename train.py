...
from adaptive_lasso import adaptive_lasso_coefficients
...
def run_for_window(returns, macro_df, window_days):
    ...
    if len(macro_window) < len(ret_window):
        return None
    raw_scores = adaptive_lasso_coefficients(ret_window, macro_window, Lambda=config.LAMBDA, gamma=config.GAMMA)
    norm_scores = normalize_scores(raw_scores)
    sorted_norm = sorted(norm_scores.items(), key=lambda x: x[1], reverse=True)
    top_etfs = [{"ticker": t, "lasso_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    out_file = f"output/adaptive_lasso_{timestamp}.json"
