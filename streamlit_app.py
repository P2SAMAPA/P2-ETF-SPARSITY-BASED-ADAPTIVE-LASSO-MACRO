...
st.sidebar.markdown(f"**λ:** {config.LAMBDA} | **γ:** {config.GAMMA}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'adaptive_lasso_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>LASSO score: {etf['lasso_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized LASSO Score"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized LASSO Score", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("📉 Top ETFs by Adaptive LASSO Score (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Adaptive LASSO** with macro‑informed penalties: ETFs with higher sensitivity to macro variables (e.g., VIX) receive lower penalty, making them more likely to be selected.
        - Score = last‑day return × macro sensitivity × (1/(1+λ)).
        - Positive score → ETF has positive momentum and is macro‑responsive → candidate for long position.
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the adaptive LASSO scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Adaptive LASSO | Macro‑informed sparsity for ETF selection")
